		...
		public static DataFile ProcessFile(string filePath)
		{
			...
			try
			{
				using (SpreadsheetDocument ss = SpreadsheetDocument.Open(filePath, false))
				{
					...
					//		// Only process sheets with all the months.
					//		var sheetNames = wbp.Workbook.Descendants<Sheet>().Select(w => w.Name.ToString()).ToList();

					//		var missing = Months.Where(m => !sheetNames.Contains(m));

					//		if (missing.Count() > 0)
					//		{
					//			Log.Msg($"Ignoring file: {fileName}");
					//			return null;
					//		}
					...
					//		foreach (var sht in wbp.Workbook.Descendants<Sheet>().Where(w => Months.Contains(w.Name)))
					//		{
					...
					//			var cols = dataCols.Select(dc => dc.col);
					//			var sref = specialCells.Where(s => s.CellReference != null).Select(s => s.CellReference);

					//			var dataSheet = dataFile.AddDataSheet(sht.Name, dataCols, specialCells);

					//			WorksheetPart wsp = wbp.GetPartById(sht.Id) as WorksheetPart;

					//			bool pass = Spreadsheet.CheckSignature(wsp.Worksheet, specialCells, dataCols, stringTable, cellFormats);
					//			if (!pass)
					...
					//							.Where(c => c.InnerText.Length > 0)
					//							.Select(t => new { cell = t, row = GetRow(t.CellReference.InnerText), col = GetColumn(t.CellReference.InnerText) })
					//							.Where(k => sref.Contains(k.cell.CellReference.InnerText) || (k.row > 10 && cols.Contains(k.col)));
					...
					//				int row = tc.row;
					//				int col = tc.col;
					//				DataColumn colmn = dataCols.Where(dc => row > 10 && dc.col == col ).FirstOrDefault();
